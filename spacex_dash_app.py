...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                dcc.Dropdown(id='site-dropdown', 
                                             ...
                                             options= [{'label': 'All Sites', 'value': 'All Sites'}]+[{'label': i, 'value': i} for i in site_list],
                                             value='All Sites',
...
def get_graph1(sitedropdown):
    if sitedropdown=='All Sites':
        pie_df = spacex_df[spacex_df['class']==1]
        fig = px.pie(pie_df, values='class', names='Launch Site', title='Total Success Launches By Site') 
    else:
        pie_df = spacex_df[spacex_df['Launch Site']==sitedropdown].groupby('class')['Mission Outcome'].size().reset_index()       
        fig = px.pie(pie_df, values='Mission Outcome', names='class', title='Total Success Launches for site '+sitedropdown)  
    return fig                     
# TASK 4:
